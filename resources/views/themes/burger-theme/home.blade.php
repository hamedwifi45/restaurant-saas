                ...
                @foreach($categories as $category)
                    <a href="{{ route('restaurant.menu', $restaurant->slug) }}#{{ $category->slug }}" 
                       class="group flex flex-col items-center p-6 bg-gray-50 rounded-2xl hover:bg-primary hover:text-white transition duration-300 shadow-sm hover:shadow-xl">
                        <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center text-3xl mb-4 group-hover:scale-110 transition shadow-sm">
...
<section class="py-16 bg-gray-50">
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-end mb-12">
            ...
            <a href="{{ route('restaurant.menu', $restaurant->slug) }}" class="btn-primary px-6 py-2 rounded-full text-sm font-bold hidden sm:inline-block">
                عرض الكل ➔
            </a>
        </div>
            ...
            @forelse($restaurant->products()->where('is_available', true)->limit(4)->get() as $product)
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden group hover:-translate-y-2 transition duration-300">
                    <div class="relative h-48 overflow-hidden">
                        ...
                        @endif
                        <span class="absolute top-4 right-4 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full">الأكثر طلباً</span>
                    </div>
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-gray-800 mb-2">{{ $product->name }}</h3>
                        <p class="text-gray-500 text-sm mb-4 line-clamp-2">{{ $product->description }}</p>
                        <div class="flex items-center justify-between">
                            <span class="text-2xl font-black text-primary">{{ number_format($product->price, 2) }} ر.س</span>
                            <form action="{{ route('cart.add', $restaurant->slug) }}" method="POST">
                                @csrf
                                <input type="hidden" name="product_id" value="{{ $product->id }}">
                                <input type="hidden" name="quantity" value="1">
                                <button type="submit" class="bg-gray-100 hover:bg-primary hover:text-white text-gray-800 w-10 h-10 rounded-full flex items-center justify-center transition">
                                    +
                                </button>
                            </form>
                        </div>
        ...
        <div class="mt-8 text-center sm:hidden">
            <a href="{{ route('restaurant.menu', $restaurant->slug) }}" class="btn-primary px-6 py-2 rounded-full text-sm font-bold inline-block">
                عرض كل القائمة
            </a>
        </div>
    </div>
